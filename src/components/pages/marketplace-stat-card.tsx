...
function KpiHeadline({ display }: { display: KpiDisplay }) {
  ...
  if (headline.kind === "plain") {
    return (
      <p className="whitespace-nowrap text-[1.75rem] font-semibold leading-none tracking-tight tabular-nums text-foreground sm:text-[2rem]">
        {headline.value}
      </p>
    );
  }
...
export function MarketplaceStatCard({
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden rounded-xl border border-foreground/10 bg-foreground/[0.03] p-4 transition-colors hover:border-foreground/20 hover:bg-foreground/[0.05]"
    >
      <div
      ...
      <div className="mt-4 pl-2">
        ...
        <p className="mt-2 text-xs leading-snug text-foreground/40">
          {display.caption}
        </p>
      </div>
    </motion.div>
